...
def minimize(X, f, args=(), maxnumlinesearch=None, maxnumfuneval=None, red=1.0, verbose=True):
    ...
    while i < abs(length):                                 # while not finished
            ...
            while (not success) and (M > 0):
                try:
                    ...
                    if isnan(f3) or isinf(f3) or any(isnan(df3)+isinf(df3)):
                        raise Exception('') #RSvB: This replicates the behavior from minimize.m, which is to bisect and try again
                        # print("error")                        
                        # return None,None,None #RSvB: The version I copied had a simple 'return' here, but minimize.m does not return here and instead raises an error and goes to the except (catch) block
                    success = 1
        ...
        if abs(d3) < -SIG*d0 and f3 < f0+x3*RHO*d0:  # if line search succeeded
            ...
            if verbose: print('{:s} {:6d}; Value {:4.6e}\r'.format(S,i,f0))
